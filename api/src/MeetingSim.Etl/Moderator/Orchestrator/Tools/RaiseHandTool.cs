...
internal sealed class RaiseHandTool : IModeratorTool
{
    public const string ToolName = "raise_hand";

    private readonly IEventPoster _poster;
    private readonly ChatTool _definition;

    public RaiseHandTool(IReadOnlyList<Persona> roster, IEventPoster poster)
    {
        _poster = poster;
        _definition = ChatTool.CreateFunctionTool(
            functionName: ToolName,
            functionDescription:
                "A persona raises their hand to signal they want to speak. They do NOT speak — they wait for "
                + "the presenter to call on them by name. Use for engagement-worthy moments: strong claims a "
                + "skeptic would probe, clarifications a curious persona wants, open questions to the room.",
